  ...
  const iconMarkup = renderToStaticMarkup(
    <div className="relative group flex flex-col items-center justify-center">
      ...
      {(isHighMatch || isSelected) && (
        <div className={`absolute -inset-2 rounded-full blur-md opacity-40 z-0 ${isSelected ? 'bg-blue-500' : 'bg-orange-500'}`}></div>
      )}
      ...
      <div className={`
        ...
        ${isSelected 
          ...
          : isHighMatch 
            ? 'bg-gradient-to-br from-orange-500 to-red-500 border-white' 
            : 'bg-white border-orange-500 text-orange-600'
        }
      `}>
        {/* Nền xanh/trắng, ruột thì đổi màu ngược lại */}
        {shop.category === 'Trà sữa' ? (
          <span className={`text-[12px] font-bold ${isSelected || isHighMatch ? 'text-white' : 'text-orange-600'}`}>🧋</span>
        ) : (
          <Coffee className={`w-4 h-4 ${isSelected || isHighMatch ? 'text-white' : 'text-orange-600'}`} strokeWidth={2.5} />
        )}
      ...
      <div className={`
        ...
        ${isSelected ? 'bg-blue-600' : isHighMatch ? 'bg-red-500' : 'bg-orange-500'}
      `}></div>
    </div>
  );
